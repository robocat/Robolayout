...
public extension UIView {
    ...
    public func constraints(vertical: String? = nil, horizontal: String? = nil, options: NSLayoutFormatOptions = [], _ views: [String : UIView]) -> [NSLayoutConstraint] {
        ...
        if let v = vertical {
            constraints += addConstraints(format: "V:\(v)", options: options, views: views)
        }
        
        if let h = horizontal {
            constraints += addConstraints(format: "H:\(h)", options: options, views: views)
        }
    ...
    private func addConstraints(format: String, options: NSLayoutFormatOptions = [], views: [String: UIView]) -> [NSLayoutConstraint] {
    ...
    public func setCenterInSuperview(offset: CGPoint = .zero, priority: UILayoutPriority = 1000) -> [NSLayoutConstraint] {
        return [
    ...
    private func setAttributeToSuperview(attribute: NSLayoutAttribute, relation: NSLayoutRelation = .equal, constant: CGFloat = 0, priority: UILayoutPriority = 1000) -> NSLayoutConstraint  {
    ...
    private func constraintUsing(_ item : UIView, _ attribute : NSLayoutAttribute, _ relation : NSLayoutRelation, _ item2 : AnyObject? = nil, _ attribute2 : NSLayoutAttribute = .notAnAttribute, multiplier : CGFloat = 1, constant : CGFloat = 0, priority : UILayoutPriority = 1000) -> NSLayoutConstraint {
